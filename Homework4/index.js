...
// //Bài 1;
{
    ...
    // 1, trong đoạn code trên thì x là property
    // 2, dùng for để đọc object trên:
...
// Bài 2:Phá hủy chuỗi object
{
    ...
    var { subject, createdBy, assignTo, dueDate, expectedHours, } = task;
    console.log(subject);
...
//Bài 3
//1, Kiểu dữ liệu lớp ngoài cùng là 1 Object
//2,Thuộc tính hits là nơi lưu trữ kết quả công việc và nó là 1 Array
//3,
...
alert('Bài 4 nhé');
{
    ...
    while (true) {
        let user = prompt('Enter a keyword');
        user = user.toLowerCase();
        if (user == false) {
            break;
        } else if (dictionary[user] == undefined) {
    ...
    while (true) {
        let userChoice = prompt('Enter a keyword');
        ...
        else if (dictionary[userChoice] == undefined) {
            let newData = prompt(`We could not find your word '${userChoice}',leave your explanation`)
            if (newData == false) {
                alert('Database update failed');
            } else {
                dictionary[userChoice] = newData;
                alert('Done');
            }
        }
        else {
            alert(`${userChoice}\n${dictionary[userChoice]}`);
        }
    }
}
// Bài 5: 
{
    alert('Bài tập số 5 nhé ==>');
    let item1 = {
    ...
    console.log(' ');
    let userChoice = prompt('HW5-Enter product position:') - 1;
    {
        for (x in products[userChoice]) {
            console.log(`${x.charAt(0).toUpperCase()}${x.slice(1)}: ${products[userChoice][x]}`);
        }
    }
    // str.slice(index) là cắt 1 phần của chuỗi để tạo 1 chuỗi mới , chuỗi ban đầu ko đổi.
    //  str.chatAt(index) : là trả lại giá trị của chuỗi được yêu cầu ở vị trí index chỉ định.

    console.log(' ');
    let user = prompt('HW5-Enter your category');

    for (i = 0; i < products.length; i++) {
        if (user.toLowerCase() == products[i].category.toLowerCase()) {
            console.log('-------------------------------');
            console.log(`Name: ${products[i].name}`);
            console.log(`Price: ${products[i].price}`);
        }
    ...
    let userProvider = prompt('HW5-Enter your provider:');
    {
        for (i = 0; i < products.length; i++) {
            for (k = 0; k < products[i]['providers'].length; k++) {
                if (userProvider.toLowerCase() == products[i]['providers'][k].toLowerCase()) {
                    console.log('-------------------------');
                    {
                        for (property in products[i]) {
                            console.log(`${property.charAt(0).toUpperCase()}${property.slice(1)}: ${products[i][property]}`);
                        }
                    }
                }
            }
...
alert('Bài 6 rồi nè')
{
    ...
    console.log(' ');
    while (true) {
        let userChoice = prompt('Enter your command (New, Delete, Update, Complete)').toLowerCase();
        if (userChoice == 'new') {
            ...
            let newUpdate = Number(prompt('Enter the position of the task')) - 1;
            let taskUpdate = prompt('Insert the new task title');
            tasks[newUpdate].name = taskUpdate;
            {
                for (i = 0; i < tasks.length; i++) {
                    if (tasks[i].Complete == false) {
                        console.log(`[ ] ${i + 1}. ${tasks[i].name}`);
...
alert('Bài 15 rồi à :( khó quá');
{
    ...
    15.6
    console.log('15.6');
    for (a = 0; a < jobData.hits.length; a++) {
        console.log(jobData.hits[a].jobTitle);
        console.log('Benefits:');
        {
            for (i = 0; i < jobData.hits[a].benefits.length; i++) {
                console.log(`- ${jobData.hits[a].benefits[i].benefitValue}`);
            }
        }
        console.log('-----------------------------');
    }
    ...
    15.7
    {
        for (a = 0; a < jobData.hits.length; a++) {
            console.log(jobData.hits[a].jobTitle);
            console.log(`Salary: $${jobData.hits[a].jobSalary}`);
            console.log('Locations:');
            {
                for (i = 0; i < jobData.hits[a].locationVIs.length; i++) {
                    console.log(`- ${jobData.hits[a].locationVIs[i]}`);
                }
            }
            console.log('Benefits:');
            {
            ...
            console.log('Skills:');
            {
                for (i = 0; i < jobData.hits[a].skills.length; i++) {
                    console.log(`- ${jobData.hits[a].skills[i]}`);
                }
            }
